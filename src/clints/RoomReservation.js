...
function RoomReservation() {
  ...
  useEffect(() => {
    // Retrieve user and room details from localStorage
    const storedUser = JSON.parse(localStorage.getItem("user"));
    const roomId = localStorage.getItem("roomId");
    const clientId = localStorage.getItem("clientId");

    if (storedUser && roomId && clientId) {
        ...
        console.error('Missing user, roomId, or clientId in localStorage');
    }
}, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.post(
            `http://localhost:9090/room_reservation?clientId=${reservationDetails.userId}&roomId=${reservationDetails.roomId}`, 
            reservationPayload
        );
